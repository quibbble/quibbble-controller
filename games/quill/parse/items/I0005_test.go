...
func Test_I0005(t *testing.T) {
	game, uuids, err := tests.NewTestEnv(tests.Player1, "I0005", "S0001")
	if err != nil {
		t.Fatal(err)
	}
	...
	assert.Equal(t, 1, len(game.Board.XYs[x][y].Unit.(*cd.UnitCard).GetTraits(tr.PurityTrait)))

	err = game.PlayCard(tests.Player1, uuids[1], u1.GetUUID())
